...
import { CalendarDays, ChevronRight, Loader2, Inbox, BookOpen } from 'lucide-react';
...
export default function Cultos() {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          evento_id,
          status,
          eventos_escala (id, titulo, data_evento, horario_inicio)
        `)
  ...
  // Quais eventos já têm liturgia criada
        ...
        <div className="space-y-6">
          ...
          {concluidos.length > 0 && (
            <section className="space-y-2">
              ...
              <div className="space-y-2">{concluidos.map(renderCard)}</div>
            </section>
          )}
        </div>
      )}
