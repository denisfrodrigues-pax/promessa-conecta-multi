...
import { Network, Download, FileText, Users, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function RelatorioBases() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const { data: basesData } = await supabase.from('bases').select('*').eq('status', 'ativo').order('nome');
      ...
      // KPIs
      const total = basesComOcupacao.length;
      const ocupacaoMedia = total > 0 ? Math.round(basesComOcupacao.reduce((s, b) => s + b.percentual, 0) / total) : 0;
      const lotadas = basesComOcupacao.filter(b => b.percentual >= 100).length;
      setKpis({ total, ocupacaoMedia, lotadas });
  ...
  const handleExportPDF = () => {
    if (reportRef.current) exportToPDF(reportRef.current, 'relatorio_bases');
  };
  ...
  return (
    <div className="space-y-6" ref={reportRef}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle className="text-base">Ocupação por Base</CardTitle></CardHeader>
          <CardContent>
      ...
      {/* Table */}
      <Card>
        <CardHeader><CardTitle className="text-base">Lista de Bases</CardTitle></CardHeader>
        <CardContent>
