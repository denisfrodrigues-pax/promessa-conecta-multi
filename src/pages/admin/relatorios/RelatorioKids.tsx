  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [criancas, presentesHoje, checkinsMes, salas] = await Promise.all([
        ...
        supabase.from("salas").select("*", { count: "exact", head: true }).eq("status", "ativa"),
      ]);
      ...
      const { count: totalCheckins } = await supabase.from("checkins_kids").select("*", { count: "exact", head: true });
      setTotal(totalCheckins || 0);

      // Paginated check-ins
      const { data: ultimosCheckins } = await supabase
        .from("checkins_kids")
        .select("*, criancas(nome), salas(nome), responsaveis(nome)")
      ...
      // Por sala
      const { data: salasData } = await supabase.from("salas").select("id, nome, capacidade");
      const salasComCheckins = await Promise.all(
  ...
  return (
    <div className="space-y-6" ref={reportRef}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-base font-bold">Check-ins - Últimos 7 dias</CardTitle>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base font-bold">Check-ins por Sala (mês)</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            <ResponsiveContainer width="100%" height={250}>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base font-bold">Últimos Check-ins</CardTitle>
        </CardHeader>
        <CardContent>
          {total > limit && (
            <div className="mb-4">
              ...
              <tbody>
                {checkins.map((c) => (
                  <tr key={c.id} className="border-b hover:bg-muted/50">
                    ...
                    <td className="p-2">{c.salas?.nome || "–"}</td>
                    <td className="p-2">{c.responsaveis?.nome || "–"}</td>
                    <td className="p-2">{format(new Date(c.checkin_at), "dd/MM HH:mm")}</td>
                    <td className="p-2">
                      <Badge
                        className={
                          c.status === "presente" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
                        }
                      >
                        {c.status}
                      </Badge>
